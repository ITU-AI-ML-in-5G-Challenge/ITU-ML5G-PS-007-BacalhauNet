...
for line in range(len(file)):
    ...
    export_base_name = f"bacalhaunetv1_quant" \
    ...
    if evaluate_acc:
        ...
        optimizer = torch.optim.Adam(bacalhaunetv1.parameters(), lr=0.01, weight_decay=weight_decay)
        ...
        for epoch in tqdm(range(num_epochs), desc="Epochs"):
            ...
            if test_acc > max_acc:
                ...
                torch.save(bacalhaunetv1.state_dict(), max_acc_save_path)
        ...
        file.loc[line, "test_accuracy"] = max_acc
        file.loc[line, "notes"] = os.path.join(export_path, export_base_name + "_Acc" + str(max_acc) + ".pth")

        torch.save(bacalhaunetv1.state_dict(), os.path.join(export_path, export_base_name + "_finalweights.pth"))
    ...
    file.loc[line, "inference_cost"] = 0.5 * (bops / bops_baseline) + 0.5 * (w_bits / w_bits_baseline)
    file.loc[line, "inference_cost_bops"] = bops / bops_baseline
    file.loc[line, "inference_cost_wbits"] = w_bits / w_bits_baseline
